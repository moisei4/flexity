    ...
    function woof_draw_select_childs(&$collector, $taxonomy_info, $term_id, $tax_slug, $childs, $level, $show_count, $show_count_dynamic, $hide_dynamic_empty_pos)
    {
        ...
        <?php if (!empty($childs)): ?>
            <?php foreach ($childs as $term) : ?>
                ...
                ?>
                <option <?php if ($show_count AND $count == 0 AND ! in_array($term['slug'], $current_request)): ?>disabled=""<?php endif; ?> value="<?php echo esc_attr($term['slug']); ?>" <?php echo selected(in_array($term['slug'], $current_request)); ?> class="woof-padding-<?php echo esc_attr($level); ?>"><?php echo str_repeat('', esc_attr($level)); ?><?php
...
<select class="woof_select woof_select_<?php echo esc_attr($tax_slug); ?>" name="<?php echo esc_attr($tax_slug); ?>">
