                ...
                <?php foreach ($childs as $term) : $inique_id = uniqid(); ?>
                    ...
                    if (!in_array($term['slug'], $current_request))
                    {
                        if ($show_count)
                        {
                            ...
                            $count_string = '<span class="woof_checkbox_count">(' . esc_attr($count) . ')</span>';
                        }
                        //+++
                    ...
                    ?>
                    <li <?php if ($WOOF->settings['dispay_in_row'][$tax_slug] AND empty($term['childs'])): ?>style="display: inline-block !important;"<?php endif; ?>><input type="checkbox" <?php if (!$count AND ! in_array($term['slug'], $current_request) AND $show_count): ?>disabled=""<?php endif; ?> id="<?php echo 'woof_' . esc_attr($term['term_id']) . '_' . esc_attr($inique_id) ?>" class="woof_checkbox_term woof_checkbox_term_<?php echo esc_attr($term['term_id']); ?>" data-tax="<?php echo esc_attr($tax_slug); ?>" name="<?php echo esc_attr($term['slug']); ?>" data-term-id="<?php echo esc_attr($term['term_id']); ?>" value="<?php echo esc_attr($term['term_id']); ?>" <?php echo checked(in_array($term['slug'], $current_request)) ?> /><label class="woof_checkbox_label <?php if (in_array($term['slug'], $current_request)): ?>woof_checkbox_label_selected<?php endif; ?>" for="<?php echo 'woof_' . esc_attr($term['term_id']) . '_' . esc_attr($inique_id); ?>"><?php
...
<ul class="woof_list woof_list_checkbox">
    ...
    <?php if (!empty($terms) AND is_array($terms)): ?>
        <?php foreach ($terms as $term) : $inique_id = uniqid(); ?>
            ...
            ?>
            <li <?php if ($this->settings['dispay_in_row'][$tax_slug] AND empty($term['childs'])): ?>style="display: inline-block !important;"<?php endif; ?>><input type="checkbox" <?php if (!$count AND ! in_array($term['slug'], $current_request) AND $show_count): ?>disabled=""<?php endif; ?> id="<?php echo 'woof_' . esc_attr($term['term_id']) . '_' . esc_attr($inique_id); ?>" class="woof_checkbox_term woof_checkbox_term_<?php echo esc_attr($term['term_id']); ?>" data-tax="<?php echo esc_attr($tax_slug); ?>" name="<?php echo esc_attr($term['slug']); ?>" data-term-id="<?php echo esc_attr($term['term_id']); ?>" value="<?php echo esc_attr($term['term_id']); ?>" <?php echo checked(in_array($term['slug'], $current_request)) ?> /><label class="woof_checkbox_label <?php if (in_array($term['slug'], $current_request)): ?>woof_checkbox_label_selected<?php endif; ?>" for="<?php echo 'woof_' . esc_attr($term['term_id']) . '_' . esc_attr($inique_id); ?>"><?php
                    if (has_filter('woof_before_term_name'))
                        echo apply_filters('woof_before_term_name', $term, $taxonomy_info);
                    else
                        echo esc_attr($term['name']);
