    ...
    function woof_draw_mselect_childs(&$collector, $taxonomy_info, $term_id, $tax_slug, $childs, $level, $show_count, $show_count_dynamic, $hide_dynamic_empty_pos)
    {
        ...
        <?php if (!empty($childs)): ?>
            <?php foreach ($childs as $term) : ?>
                ...
                ?>
                <option <?php if ($show_count AND $count == 0 AND ! in_array($term['slug'], $current_request)): ?>disabled=""<?php endif; ?> value="<?php echo esc_attr($term['slug']); ?>" <?php echo selected(in_array($term['slug'], $current_request)) ?>><?php echo str_repeat('&nbsp;&nbsp;&nbsp;', $level) ?><?php
                    if (has_filter('woof_before_term_name'))
                        echo apply_filters('woof_before_term_name', $term, $taxonomy_info);
                    else
                        echo esc_attr($term['name']);
                    ?> <?php echo wp_kses_post($count_string); ?></option>
                <?php
...
<select class="woof_mselect woof_mselect_<?php echo esc_attr($tax_slug); ?>" data-placeholder="<?php echo WOOF_HELPER::wpml_translate($taxonomy_info) ?>" multiple="" size="<?php echo($this->is_woof_use_chosen() ? 1 : '') ?>" name="<?php echo esc_attr($tax_slug); ?>">
    ...
    <?php if (!empty($terms)): ?>
        <?php foreach ($terms as $term) : ?>
            ...
            ?>
            <option <?php if ($show_count AND $count == 0 AND ! in_array($term['slug'], $current_request)): ?>disabled=""<?php endif; ?> value="<?php echo esc_attr($term['slug']); ?>" <?php echo selected(in_array($term['slug'], $current_request)) ?>><?php
                if (has_filter('woof_before_term_name'))
                    echo apply_filters('woof_before_term_name', $term, $taxonomy_info);
                else
                    echo esc_attr($term['name']);
...
<?php if ($shown_options_tags == 0): ?>
    <style type="text/css">
        .woof_container_<?php echo esc_attr($tax_slug); ?>{
            display:none;
        }
    </style>
<?php endif; ?>
...
if (!empty($collector))
{
    foreach ($collector as $ts => $values)
    {
        if (!empty($values))
        {
            ...
            {
                ?>
                <input type="hidden" value="<?php echo esc_attr($value['name']); ?>" data-anchor="woof_n_<?php echo esc_attr($ts); ?>_<?php echo esc_attr($value['slug']); ?>" />
                <?php
            }
        }
    }
